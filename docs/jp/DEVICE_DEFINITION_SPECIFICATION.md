...
- `ui.configuration_fields` には任意の平文 `description` を指定できます。
  Hub は説明をエスケープし、`aria-describedby` で入力欄に関連付けます。
  製品ごとの設定説明は Device Definition が所有します。
...
`ui.threshold_rules` は有効化・しきい値・判定元の3つの設定パスをまとめて表示します。
判定元フィールドの型は `measurement_source`、値は `first`・`average`・
`sensor:<baud>:<address>` です。ルールは状態データ内のセンサー一覧、対象種別、
有効・読取成功・測定値・名前・場所・識別子のキー、単一センサー時の参照先を宣言します。
登録有無が不明なときは代替値を推測せず、欠損値を0として表示しません。
平均の対象が1台でも読めなければ、平均も取得できないと表示します。

説明、機器が報告した設定と対応可否、前回判断の値・しきい値・結果の参照先も定義します。
Hubは選択対象、受信時刻、編集中の比較、前回の実際の判断を区別して表示します。
運転判断と読取失敗時の動作はfirmwareが所有し、画面から運転は開始しません。
表示名と説明はすべてエスケープします。

`runtime_config.capability_requirements` は `enabled_path`・`status_path`・
平文の `message` を持ちます。有効な機能の対応可否を最新statusで確認できるまで、
MQTTのreplyとpushを拒否し、旧firmwareが新設定を無視することを防ぎます。
