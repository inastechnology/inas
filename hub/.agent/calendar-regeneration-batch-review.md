...
変更案が多い場合でも内容を読み飛ばさず判断できるように、変更案カードから作業詳細相当の比較画面を開き、1件ずつ「取り入れる」「取り入れない」を選ぶ。判断後は次の未確認案を自動表示し、最後にまとめて反映する連続レビューにする。
...
- [x] (2026-07-20) 一括反映までを `f6ee7cb` としてコミットした。
- [x] (2026-07-20) 既存の作業詳細表示を再利用した変更前・AI提案の比較画面を追加した。
- [x] (2026-07-20) 変更案カードから開始し、判断後に次の未確認案へ進む連続レビューを追加した。
- [x] (2026-07-20) 連続レビューと最終一括通信をブラウザsmokeで検証した。
- [x] (2026-07-20) 連続レビュー実装をコミットし、稼働Hubへ反映した。
...
- `CalendarActionCard` の作業詳細は期間、作業量、判断の流れ、作業メモを既に表示している。編集操作を含まない読み取り専用previewを同じモジュールから公開すれば、比較画面でも同じ情報設計を利用できる。
...
- 変更案の通常表示は件数と進捗が分かるコンパクトなカード一覧にする。カードを押すと選んだ案から比較を開始し、決定後は次の未確認案を自動表示する。
- 比較画面はデスクトップでは「現在」「AIの提案」を横並び、モバイルでは縦並びにし、既存の作業詳細相当の情報を読み取り専用で表示する。追加と削除では存在しない側を明示する。
- 最後の判断後も即時保存はせず、完了画面で件数を確認してから一括反映する。途中で閉じても選択は保持し、一覧カードから個別に見直せる。
...
`CalendarActionCard.tsx` に編集操作を含まない作業previewを追加する。`PlantCalendarDrawer.tsx` は変更案一覧をレビュー開始カードへ変更し、比較dialog、進捗、決定後の自動遷移、全件終了画面を追加する。既存のローカル選択stateと一括APIはそのまま利用する。
...
- 連続レビュー追加後もTypeScript型チェック、本番ビルド、Pythonの全344テストが成功した。
- 更新案では現在とAI提案の作業詳細が左右に表示され、追加案では現在側が空である理由とAI提案の作業詳細が表示されることをChromiumで確認した。
- 1件目の判断後に2件目へ自動遷移し、各判断時の決定APIは0回、完了画面の一括反映時のみ2件を含むPOSTが1回であることをChromium smokeで確認した。
- モバイルでは比較が縦並びになり、判断ボタンが画面下部に表示され、横方向のはみ出しがないことを確認した。
- 稼働Hubを再起動し、`healthz` が `ok`、`readyz` がweb・MQTTともにtrueであること、新しい管理画面bundleから連続レビューの文言が配信されることを確認した。
...
Revision note (2026-07-20): 変更案カードから作業詳細相当の比較を開き、1件ずつ連続判断する追加要件と設計を追記した。

Revision note (2026-07-20): 連続レビューを実装し、デスクトップ・モバイルの表示と通信回数の検証結果を追記した。

Revision note (2026-07-20): コミットと稼働Hubへの反映、ヘルスチェック結果を追記した。
