...
圃場 (Field)
  空間 (LayoutSpace: ハウス、露地、屋内、水耕設備)
    配置物 (Placement)
      ...
      設備: 潅水機、センサー、タンク、植物育成ライト、噴霧器、送風機、空調、任意のホース・配管
...
培地に `waters` 相当の関連がない状態は `手動潅水（ホース等）` とする。ホース・配管の配置物は設置図上の物理経路であり、制御主体ではない。自動潅水へ切り替える場合は、配置済み潅水機の `binding.target_placement_ids` に培地IDを追加する。
