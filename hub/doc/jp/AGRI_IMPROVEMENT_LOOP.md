...
- `location`: 都道府県、市区町村、町名・地区、設置環境（屋外、ハウス内、屋内、半屋外、その他）
...
圃場の新規作成では `name` と `location` だけを登録する。作物と栽培条件は圃場詳細、具体的なハウス、畝、設備、デバイスは設置ビューまたはデバイス関連設定で管理し、圃場作成フォームへ混在させない。
...
2. 前提をそろえる
   - 作物名、品種、作物区分、樹齢、生育段階、栽培方式、土壌・培地、株数を設置ビュー上の定植単位に登録する。地域は圃場所在地から取得する。
   - 定植時に作物と栽培条件に応じた土壌水分、EC、pH、湿度、光量の目標レンジをAIまたは標準案で生成し、利用者が修正する。
...
4. アクションを記録する
   ...
   - 現行の「次の判断候補」は圃場 TODO の初版とし、将来は植物管理カレンダー、画像・気象判断、設備保守、定期作業、利用者登録作業を同じリストへ統合する。

5. 実施と評価を残す
   - 灌水、液肥、噴霧、追肥、遮光、収穫などの作業は圃場イベントとして記録する。
   - 実施後の人間評価は1から5の値と5段階絵文字で残し、文章の観察メモとは分離する。
   - 作業・観察画像はR2へ保存し、日付単位の記録から振り返れるようにする。
