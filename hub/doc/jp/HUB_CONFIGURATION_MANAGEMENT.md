...
| AI栽培計画プロンプトフォーマット | GUI・上級者向け | 栽培計画固有の入力順序と追加指示を運用中に調整する |
...
AI栽培計画プロンプトは `config.json` の非秘密ランタイム設定として保存する。テンプレートには `{default_instructions}`、`{context_json}`、`{guidance_json}` が必須で、`{experience_instruction}` は任意とする。保存時に必須プレースホルダーと12,000文字上限を検証し、手動編集された設定ファイルが不正な場合は実行時に内蔵テンプレートへ戻す。カスタムテンプレートはユーザーメッセージだけを構成し、システムメッセージの安全条件、過去日禁止、出力JSON検証、農薬・資材の根拠制約は変更できない。
...
- `AI`: テキスト・画像AIの設定、書き込み専用のAPIキー入力、接続確認、上級者向け栽培計画プロンプト。
