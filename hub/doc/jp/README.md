...
起動後に `http://127.0.0.1:39251/demo/mqtt-devices` を開きます。一覧カードで灌水・土壌水分・次回起床のサマリを確認し、カードを選ぶと水やり機詳細へ遷移します。詳細では Plotly で灌水推移と土壌水分推移を確認でき、表示期間は直近3日、2週間、1か月、全期間、カスタムから選べます。確認サーバは `.env` の実Turso URLを継承せず、`HUB_DEMO_WORK_DIR` 配下のローカルlibSQLだけを使用します。デモページ上の操作は実データへ保存されません。実データの管理画面は通常通り `/mqtt-devices` です。
