...
FIELDS = (
    ...
    ConfigurationField("HUB_READINESS_TIMEOUT_SECONDS", "起動準備タイムアウト（秒）", "core", "600", True),
    ...
    ConfigurationField("TURSO_SYNC_ON_WRITE", "書き込みごとに同期（remote遅延時は非推奨）", "turso", "false"),
    ...
    ConfigurationField("HEALTHCHECK_RECOVERY_ENABLED", "host自動復旧を有効化", "notification", "true"),
    ConfigurationField("HEALTHCHECK_DISCORD_ENABLED", "host healthcheckのDiscord通知", "notification", "true"),
    ConfigurationField("HEALTHCHECK_FAILURE_THRESHOLD", "復旧開始までの連続失敗回数", "notification", "3"),
    ConfigurationField("HEALTHCHECK_NETWORK_MANAGER_THRESHOLD", "NetworkManager再起動までの連続失敗回数", "notification", "6"),
    ConfigurationField("HEALTHCHECK_REBOOT_AFTER_FAILURES", "host再起動までの連続失敗回数（0で無効）", "notification", "0"),
    ConfigurationField("HEALTHCHECK_ACTION_COOLDOWN_SECONDS", "復旧操作の再実行間隔（秒）", "notification", "300"),
    ConfigurationField("HEALTHCHECK_REBOOT_COOLDOWN_SECONDS", "host再起動の再実行間隔（秒）", "notification", "21600"),
    ConfigurationField("HEALTHCHECK_REQUEST_TIMEOUT_SECONDS", "healthcheck通信timeout（秒）", "notification", "5"),
    ConfigurationField("HEALTHCHECK_HEARTBEAT_INTERVAL_SECONDS", "正常確認通知の間隔（秒）", "notification", "21600"),
    ConfigurationField("HEALTHCHECK_BOOT_NOTIFICATION_ENABLED", "起動後の正常確認を通知", "notification", "true"),
    ConfigurationField("HEALTHCHECK_BOOT_GRACE_SECONDS", "起動直後の復旧猶予（秒）", "notification", "600"),
    ConfigurationField("HEALTHCHECK_NETWORK_INTERFACE", "監視network interface", "notification", "wlan0"),
    ConfigurationField("HEALTHCHECK_NETWORK_CONNECTION", "復旧するNetworkManager接続名", "notification"),
    ConfigurationField("HEALTHCHECK_EXTERNAL_HOST", "外部疎通確認host", "notification", "archive-api.open-meteo.com"),
    ConfigurationField("HEALTHCHECK_EXTERNAL_PORT", "外部疎通確認port", "notification", "443"),
    ConfigurationField("HEALTHCHECK_CAMERA_ENABLED", "登録cameraの到達確認", "notification", "true"),
    ConfigurationField("HEALTHCHECK_TUNNEL_ENABLED", "Cloudflare Tunnelの到達確認", "notification", "true"),
    ConfigurationField("HEALTHCHECK_TUNNEL_READY_URL", "Tunnel readiness URL", "notification", "http://127.0.0.1:20241/ready"),
