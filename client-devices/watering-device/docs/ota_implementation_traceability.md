...
| 7. MQTT Topics | Device processes only own `ota/reply` and `ota/push` | common `app_network_sub_callback()` checks topic device id and routes OTA offers to `app_ota_apply_offer_json()` | `make build` |
