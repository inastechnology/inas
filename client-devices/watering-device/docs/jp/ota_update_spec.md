...
-D APP_FIRMWARE_PROJECT=\"watering-device\"
-D APP_FIRMWARE_TARGET=\"seeed_xiao_esp32s3\"
...
- The firmware binary must embed the `INAS_FW_MANIFEST_V1` block generated by
  common firmware code. Hub upload scans this manifest and rejects binaries
  whose embedded `device_kind`, `version`, or `build_id` conflicts with the
...
- Firmware manifest extraction from uploaded `firmware.bin`
- Rejection of uploads whose embedded `device_kind`, `version`, or `build_id`
  conflicts with the requested artifact metadata
...
6. Confirm the registered artifact URL is `http://<hubのドメイン名またはIPアドレス>:39151/firmware/WTR/1.1.0/firmware.bin` or the equivalent resolved firmware base URL.
7. Confirm artifact metadata includes `device_kind: "WTR"` and `firmware_metadata`.
8. Assign `target_firmware_version` to selected active devices.
9. Monitor `ota/status` and normal status after reboot.
9. Expand rollout.
