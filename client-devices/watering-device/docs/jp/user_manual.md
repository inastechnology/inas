...
- 土壌水分がしきい値未満の場合のみ灌水
- RS485 Modbusセンサーを接続した場合、起床ごとに光量、地温、土壌EC、pH、NPKなどを測定
- RS485センサー用12V電源をMOSFETでON/OFFし、Deep Sleep中はセンサーへ常時通電しない
...
5. NTPで時刻同期します。
6. 土壌水分を測定します。
7. RS485センサーが有効な場合、12Vセンサー電源をONにし、安定待ち後に値を読み取り、電源をOFFにします。
8. 現在時刻に該当する灌水スケジュールがあるか確認します。
9. 灌水スケジュールがあり、土壌水分がしきい値未満の場合、指定チャンネルを指定秒数だけ動作させます。
10. ネットワーク接続中であれば、statusをMQTTへpublishします。
11. `debug_log_on_wake`が有効な場合、debug logをMQTTへpublishします。
12. 次回スケジュールまでdeep sleepします。
...
灌水スケジュールがない起床でも、土壌水分の測定値はstatusの`last_soil_moisture`として送信されます。RS485センサーが有効な場合、灌水とは無関係の起床でも環境値を送信します。

RS485センサー電源の注意:

- 対象は12Vで動くRS485センサーです。
- MOSFETで切るのは、RS485センサーへ向かう12V分岐です。
- ESP32S3本体の電源は、通常どおり12Vから5Vへ降圧したDCDC経路で供給します。
- 12V電源投入後の待ち時間はHubの「環境センサー 詳細設定」で調整できます。
...
| `env_sensors.par.enabled` | 任意 | `true` / `false` | RS485光量センサーを使うか |
...
- `schedules`は最大8件までです。
- Hubが配布するruntime config payloadは4096 bytes未満にしてください。
...
| `sensor_12v_power_requested` | RS485センサー電源をONにしようとしたか |
| `sensor_12v_power_configured` | センサー電源MOSFETピンが設定されているか |
| `sensor_12v_power_error` | センサー電源ONに失敗したか |
| `par_ok` / `soil_rs485_ok` | RS485センサーの読み取りに成功したか |
| `par_umol_m2_s` | 光合成に使える光量 |
| `soil_moisture_percent` | RS485土壌センサーの土壌水分 |
| `soil_temperature_c` | 地温 |
| `soil_ec_us_cm` | 土壌EC |
| `soil_ph` | 土壌pH |
| `soil_n_mg_kg` / `soil_p_mg_kg` / `soil_k_mg_kg` | 窒素、リン、カリウム |
