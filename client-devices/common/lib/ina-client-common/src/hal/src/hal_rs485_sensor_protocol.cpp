...
static bool read_soil_sensor(const hal_rs485_soil_sensor_config_t *config,
                             ...
                             hal_rs485_soil_sample_t *out_sample)
{
    ...
    if (!ok)
    {
        Serial.printf("RS485 soil sensor read failed: slave=%u function=0x%02X start=0x%04X count=%u elapsed=%lu ms\n",
                      static_cast<unsigned int>(request.slave_id),
                      request.function_code,
                      request.start_register,
                      static_cast<unsigned int>(request.register_count),
...
bool hal_rs485_soil_sensor_read(const hal_rs485_soil_sensor_config_t *config,
                                hal_rs485_soil_sample_t *out_sample)
{
    return read_soil_sensor(config, 7, out_sample);
}
...
bool hal_rs485_soil_moisture_temperature_ec_sensor_read(
    ...
    hal_rs485_soil_sample_t *out_sample)
{
    return read_soil_sensor(config, 3, out_sample);
}
