...
bool AppDevice::check_ota_update(uint32_t seq_id) const
{
    ...
    Serial.printf("Waiting for retained OTA offer for %u ms\n", static_cast<unsigned int>(APP_OTA_OFFER_WAIT_MS));
    const bool ota_offer_received = app_ota_is_offer_received() || app_network_wait_for_ota_offer(APP_OTA_OFFER_WAIT_MS);
