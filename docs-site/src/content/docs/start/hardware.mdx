...
:::caution[技術者向け]
このページは、Raspberry Piと電子部品を自分で選定・構築する人向けです。提供機器を通常利用する場合は、[利用方法を選ぶ](/start/choose-path/)へ戻ってください。
:::
...
<figure class="concept-illustration">
  <a href="/images/illustrations/hardware-overview.webp" aria-label="Hubと圃場デバイスの機器構成スケッチを原寸で開く">
    <img src="/images/illustrations/hardware-overview.webp" alt="Raspberry Pi、ストレージ、電源、ルーターと、ESP32S3、センサー、潅水部品を左右に並べたスケッチ" width="1216" height="704" loading="lazy" decoding="async" />
  </a>
  <figcaption>左が常時稼働するHub側、右が圃場へ置くデバイス側の購入物イメージです。実際の型式と定格は、このページの表と各デバイスの部品表で確定します。</figcaption>
</figure>
...
<ManualVisual kind="raspberry-setup" label="パソコン、記録カード、Raspberry Pi本体を購入して基本ソフトウェアを書き込む構成略図" />
...
<ManualVisual kind="dry-place" label="水や薬剤から離れた屋内の高く乾燥した場所へRaspberry Pi Hubを置く略図" />
...
<ManualVisual kind="wifi-kit" label="ルーター、Raspberry Pi Hub、管理用パソコン、圃場機器の通信距離を購入前に確認する略図" />
...
<ManualVisual kind="self-build" label="圃場機器の基板、外部ドライバー、電源、センサー、測定工具をそろえる略図" />
...
購入品が揃ったら、次は[通信・ネットワークの技術詳細](/technical/networking/)で構成を決めます。
